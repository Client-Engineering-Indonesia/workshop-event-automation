...
i=0
echo "Waiting for the ACE Dashboard instance to get ready..."
while [ "$(oc get dashboard --no-headers -n tools | awk '{print $5}')" != "Ready" ] && [ $i -lt 15 ]
do
...
if [ "$(oc get dashboard --no-headers -n tools | awk '{print $5}')" != "Ready" ]
then
    ...
    curl --ssl-reqd \
         ...
         --upload-file email-files/09a-ace-dashboard-inst-deploy-failure.txt
else
    echo "ACE Dashboard is Ready."
    curl --ssl-reqd \
         --url "smtp://smtp.mailtrap.io:2525" \
         --user "${MAILTRAP_USER}:${MAILTRAP_PWD}" \
         --mail-from cp4i-admin@ibm.com \
         --mail-rcpt cp4i-user@ibm.com \
         --upload-file email-files/09a-ace-dashboard-inst-deploy-success.txt
fi
echo "Done!"