...
oc apply -f instances/${CP4I_VER}/${OCP_TYPE}/03-operations-dashboard-instance.yaml
i=0
while [ $(oc get operationsdashboard --no-headers -n tools | awk '{print $2}') != "Ready" ] && [ $i -lt 15 ]
do
...
if [ $(oc get operationsdashboard --no-headers -n tools | awk '{print $2}') != "Ready" ]
then
    ...
    curl --ssl-reqd \
         ...
         --upload-file email-files/06-operations-dashboard-inst-deploy-failure.txt
else
    ...
    curl --ssl-reqd \
         ...
         --mail-from cp4i-admin@ibm.com \
         --mail-rcpt cp4i-user@ibm.com \
         --upload-file email-files/06-operations-dashboard-inst-deploy-success.txt
fi
echo "Done!"