...
echo "Deploying Asset Repo instance..."
oc apply -f instances/${CP4I_VER}/${OCP_TYPE}/02-asset-repo-ai-instance.yaml
i=0
echo "Waiting for the Asset Repo instance to get ready..."
while [ $(oc get assetrepository --no-headers -n tools | awk '{print $2}') != "Ready" ] && [ $i -lt 15 ]
do
...
if [ $(oc get assetrepository --no-headers -n tools | awk '{print $2}') != "Ready" ]
then
    ...
    curl --ssl-reqd \
         ...
         --upload-file email-files/05-asset-repo-inst-deploy-failure.txt
else
    echo "Asset Repo is Ready."
    curl --ssl-reqd \
         --url "smtp://smtp.mailtrap.io:2525" \
         --user "${MAILTRAP_USER}:${MAILTRAP_PWD}" \
         --mail-from cp4i-admin@ibm.com \
         --mail-rcpt cp4i-user@ibm.com \
         --upload-file email-files/05-asset-repo-inst-deploy-success.txt 
fi
echo "Done!"