...
then
    ...
    then
        ...
        then
            ...
            rm -f artifacts/MAU-Demo-App.json
        ...
        APP_URL=$(jq -r '.url' "artifacts/MAU-Demo-App.json")
        # Dflt App
        ( echo "cat <<EOF" ; cat templates/template-apic-app.json ;) | \
            APP_NAME='MAU-Dflt-App' \
            APP_TITLE='MAU Dflt App' \
            PORG_URL=${PORG_URL} \
            CATALOG_URL=${CATALOG_URL} \
            CONSUMER_ORG_URL=${CONSUMER_ORG_URL} \
            sh > dflt-app.json
        ...
        then
            ...
            rm -f artifacts/MAU-Dflt-App.json
        ...
        # Prem App
        ( echo "cat <<EOF" ; cat templates/template-apic-app.json ;) | \
            APP_NAME='MAU-Prem-App' \
            APP_TITLE='MAU Prem App' \
            PORG_URL=${PORG_URL} \
            CATALOG_URL=${CATALOG_URL} \
            CONSUMER_ORG_URL=${CONSUMER_ORG_URL} \
            sh > prem-app.json
        ...
        then
            ...
            rm -f artifacts/MAU-Prem-App.json
